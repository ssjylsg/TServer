...
var gaodeCurrentCustomId = '';

var GaodeStyleEditor = {
	dom: {
	...
	tpl: '<div id="rule-item-#{index}" data-id = "#{index}" class="rule-item animated">' +
		...
		'<table width="100%">' +
		'<tr><td>属性:<span>#{elementTypeName}</span></td></tr></table>' +
		'<table width="100%">' +
		'<tr><td  width="50%">颜色:<span><em style="background:#{color}"></em>#{color}</span></td>' +
		'<td>显示:<span>#{visibility}</span></td>' + 
		'</tr>'+
	...
	init: function() {
		var me = this;
		this.bind();
		var mapstyleStr = localStorage.getItem('gaodeMapstyle');
		if (mapstyleStr && mapstyleStr.length > 3) {
			this.setMapJson(JSON.parse(localStorage.getItem('gaodeMapstyle')));
		} else {
			this.add();
		}
	...
	preview: function() {
		...
		this._previewTimer = setTimeout(function() {
			...
			localStorage.setItem('gaodeMapstyle', JSON.stringify(me.getJson()));
			map.setMapStyle({
...
var zTree;
initGaodeCatalogTree();

GaodeStyleEditor.init();

function initGaodeCatalogTree() {
	var setting = {
		view: {
			showIcon: false,
			nameIsHTML: true,
