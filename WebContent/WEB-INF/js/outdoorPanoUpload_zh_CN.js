...
$(function() {
	// 场景文本框提示
	$('#upload-realname').attr('placeholder','填写场景名称');
	initMap();
	initUpload();
});
...
function verifyImg() {
	...
	if (files.length !== 1) {
		$().toastmessage('showErrorToast', '只能上传一张图片 !');
		initPickInput();
		return;
	}
	...
	if (type.indexOf('jpeg') === -1) {
		$().toastmessage('showErrorToast', '图片格式只能为 jpg !');
		initPickInput();
	} else {
		// 判断图片宽高比
		verifyProportion();
	...
	reader.onload = function(e) {
		...
		image.onload = function() {
			...
			if (proportion === 2) {
				...
				$().toastmessage('showErrorToast', '图片宽高比只能等于 2:1 !');
				initPickInput();
			}
		};
		image.src = data;
...
function preivew() {
	...
	if ('' === imgURL) {
		$().toastmessage('showErrorToast', '不支持图片预览 !');
	} else {
...
function uploadImg() {
	...
	if ('' === realName) {
		$().toastmessage('showErrorToast', '场景名称必须填写 !');
		return;
	}
