function removeServiceConfig(that, name,mapType) {
		$.post('/TServer/admin/removeServiceConfig', {
			...
			if (result.isSucess) {
				...
				$().toastmessage('showErrorToast', 'Successfully deleted!');
			} else {
	...
	$(function() {
		...
		$("#mapConfig").dialog({
			...
			buttons: {
				"Add": function() {
					...
					if (mapName === '') {
						$().toastmessage('showErrorToast', 'Name is required!');
						return;
					}
					var reg = new RegExp('^[A-Za-z]+$');
					if (!reg.test(mapName)) {
						$().toastmessage('showErrorToast', 'Name must be in English!');
						return;
					}

					if (mapTitle === '') {
						$().toastmessage('showErrorToast', 'Title is required!');
						return;
					}
					if (titleUrl === '') {
						$().toastmessage('showErrorToast', 'Please select the slice address!');
						return;
					}
					var data = {
						name: mapName,
		...
		$("#btnAddMap").click(function() {
			...
			$.ajax({
				...
				success: function(result) {
					if ('' === result) {
						$().toastmessage('showErrorToast', 'No map tiles!');
					}
				...
				error: function() {
					$('#mapTitleName').append('<option value="">Select</option>');
				}
			...
			function() {
				...
				$.post('/TServer/admin/testDb', postData, function(result) {
					if (result.isSucess) {
						...
						$().toastmessage('showErrorToast', "Database configuration error!");
					}
				}, 'json');
		...
		//$("select[name='selectTableName']").bind('change',
				function() {
					...
					if (id === 'roadnet') {
						...
						tableName = "indoor panorama point";
						if(selectValue === '') {
						...
						tableName = "indoor panorama configuration";
						if(selectValue === '') {
					...
					var msg = "The "+tableName+" (" + selectValue + ") table has been selected";
					if (selectValue.length === 0) {
						msg = "There is no " +tableName+ " table";
					} else {
						if (id === 'panoconfig' || id === 'snpanopoint' || id === 'snpanoconfig') {
							msg = "The " + tableName + " (" + selectValue + ") table has been selected to continue adding data";
						}
		...
		$('#input-district').on('filebatchselected', function(event, files) {
			...
			if(!verify) {
				...
				$().toastmessage('showErrorToast', 'Import files and tables do not match!');
			} else {
		...
		$('#input-district').on("filepreupload", function(event, data, previewId,
			index) {
			$('#input-district-msg').html("Being imported...");
		});
		...
		$('#input-roadnet').on("filepreupload", function(event, data, previewId,
			index) {
			$('#input-roadnet-msg').html("Being imported...");
		});
		...
		$('#input-roadcross').on("filepreupload", function(event, data, previewId,
			index) {
			$('#input-roadcross-msg').html("Being imported...");
		});
		...
		$('#input-poi').on("filepreupload", function(event, data, previewId,
			index) {
			$('#input-poi-msg').html("Being imported...");
		});
		...
		$('#input-road').on("filepreupload", function(event, data, previewId,
			index) {
			$('#input-road-msg').html("Being imported...");
		});
		...
		$('#input-panoconfig').on("filepreupload", function(event, data, previewId,
			index) {
			$('#input-panoconfig-msg').html("Being imported...");
		});
        ...
        $('#input-snpanopoint').on("filepreupload", function(event, data, previewId,
            index) {
            $('#input-snpanopoint-msg').html("Being imported...");
        });
        ...
        $('#input-snpanoconfig').on("filepreupload", function(event, data, previewId,
            index) {
            $('#input-snpanoconfig-msg').html("Being imported...");
        });
	});
	...
	// 根据表类型加载所有表信息
	function load() {
		var empOption = '<option value="">Select</option>';
		...
		$.ajax({
			...
			error: function() {
                ...
                $().toastmessage('showErrorToast', 'Error loading database table information!');
			}
		});
	}
	...
	// 新增模型服务界面
	$("#modelConfig").dialog({
		title: "Added model configuration",
		autoOpen: false,
		modal: true,
		width: '400px',
		buttons: {
			"Add": function() {
				...
				if (titleUrl === '') {
					$().toastmessage('showErrorToast', 'Please select the model address!');
					return;
				}
				var data = {
					name: mapName,
	...
	$("#btnAddModel").click(function() {
		...
		$.ajax({
			...
			success: function(result) {
				if ('' === result) {
					$().toastmessage('showErrorToast', 'No model file!');
				}
				var opts = '<option value="">Select</option>';
				if (result.length !== 0) {
					var jsonArray = JSON.parse(result);
			...
			error: function() {
				$('#modelTitleName').append('<option value="">Select</option>');
			}
	...
	function modelAttributesDataAppend(name) {
        ...
        $('#input-model').on("filepreupload", function(event, data, previewId,
            index) {
            $('#input-model-msg').html("Being imported...");
        });
    }